...
def func0(x, s1, s2, mu1, mu2, a1, a2):
	import numpy as np
	return gauss0(x, s1, mu1, a1) + gauss0(x, s2, mu2, a2)
...
functions = {'gauss0' : gauss0, 'gauss2' : gauss1, 'gauss2' : gauss2, 'func0' : func0, 'func1': func1, 'func2' : func2}

def g_guess(mbins):
	import numpy as np
	from sys import exit
	g = np.array( [ [.2, .4, 1.7, 2.475, 100., 700.], [.2, .15, 1.75, 2.475, 2000., 6000.], [.6, .15, 1.9, 2.55, 9000., 16000.], [.3, .15, 1.7, 2.5, 7500., 13000.], [.3, .2, 1.5, 2.25, 5000., 6200.], [.3, .3, 1.4, 2.5, 10000., 8000.], [.35, .175, 1.35, 2.25, 1600., 800.], [.2, .5, 1.3, 2., 1700., 90.] ] ) # USE THIS AS DEFAULT
	if g.shape == (mbins, 6):
		return g
	else:
...
def fitmagbin(data, mbin, cbins, iteration, **kwargs):
	import numpy as np
	...
	if population == 'blue':
		opt, cov = curve_fit(gauss0, bincenters, N, p0 = g_guess[mbin, 0::2], sigma = E, maxfev = 100000)
...
def plotmagbin(data, best_fit, iteration, mbin, E, bincenters):
	...
	gauss_1 = gauss0(x, best_fit[iteration, mbin, 0], best_fit[iteration, mbin, 2], best_fit[iteration, mbin, 4])
	gauss_2 = gauss0(x, best_fit[iteration, mbin, 1], best_fit[iteration, mbin, 3], best_fit[iteration, mbin, 5])
	plt.plot(x, gauss_1, 'b--', x, gauss_2, 'r--', x, gauss_1 + gauss_2, 'g')
	
	
	ggauss1 = gauss0(x, g_guess[mbin, 0], g_guess[mbin, 2], g_guess[mbin, 4])
	ggauss2 = gauss0(x, g_guess[mbin, 1], g_guess[mbin, 3], g_guess[mbin, 5])
...
def tanhlinfit(blue_col, red_col):
	from scipy.optimize import curve_fit
	print 'blue:'
	print OPT[iteration, : , blue_col]
	print ERR[iteration, : , blue_col]

	print 'red:'
	print OPT[iteration, : , red_col]
	print ERR[iteration, : , red_col]
...
for index, b in enumerate(binned_data):
	...
	sg_prompt = raw_input('Should I try a single gaussian instead?')
	if int(sg_prompt) == 1:
		color_prompt = raw_input('red or blue?')
		if color_prompt == 'red':
			
		elif color_prompt == 'blue:

		else:
			print 'Invalid color: choose red or blue next time'
			exit()
...
import numpy as np
from scipy.optimize import curve_fit
bluefit_a, bluefit_a_e = curve_fit(gauss0, middles, OPT[2, : , 4], p0 = (1., -19.6, 13500.) )
redfit_a, redfit_a_e = curve_fit(gauss0, middles, OPT[2, : , 5], p0 = (1., -20.5, 18500.) )
...
plt.plot(x, gauss0(x, bluefit_a[0], bluefit_a[1], bluefit_a[2]), c = 'b')
plt.plot(x, gauss0(x, redfit_a[0], redfit_a[1], redfit_a[2]), c = 'r')
...
A = np.array( [ gauss0(middles, bluefit_a[0], bluefit_a[1], bluefit_a[2]), gauss0(middles, redfit_a[0], redfit_a[1], redfit_a[2]) ] )
...
def f_r(r, c):
	import numpy as np
	a = gauss0(r, redfit_a[0], redfit_a[1], redfit_a[2])
	...
	return gauss0(c, s, m, a)

def f_b(r, c):
	import numpy as np
	a = gauss0(r, bluefit_a[0], bluefit_a[1], bluefit_a[2])
